...
echo "addnode=tarmoilves.eu" | sudo tee -a $config
echo "addnode=node.gridcoin.network" | sudo tee -a $config
echo "addnode=de.gridcoin.pl" | sudo tee -a $config
...
echo "addnode=is.gridcoin.pl" | sudo tee -a $config
echo "addnode=nl.gridcoin.pl" | sudo tee -a $config
echo "addnode=pl.gridcoin.pl" | sudo tee -a $config
echo "addnode=gridcoins.org" | sudo tee -a $config
echo "addnode=london.grcnode.co.uk" | sudo tee -a $config
echo "addnode=quebec.gridcoin.co.il" | sudo tee -a $config
echo "addnode=seeds.gridcoin.ifoggz-network.xyz" | sudo tee -a $config
echo "addnode=toronto01.gridcoin.ifoggz-network.xyz" | sudo tee -a $config
echo "addnode=vancouver01.gridcoin.ifoggz-network.xyz" | sudo tee -a $config
echo "addnode=ils.gridcoin.co.il" | sudo tee -a $config
echo "addnode=grcexplorer.neuralminer.io" | sudo tee -a $config
echo "addnode=grcnode.nems.space" | sudo tee -a $config
...
echo "addnode=seattle.gridcoin.stablenode.net" | sudo tee -a $config
